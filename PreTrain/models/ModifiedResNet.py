...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=2048, width=64):
        super().__init__()
        self.output_dim = output_dim
        self.input_resolution = input_resolution

        # the 3-layer stem 3(2)>3(1)>3(1) my: 64(2)>64(2)>64(1) / 3(2)>3(2)>3(1) / 64(4)>64(1)>64(1) 很多组合，待考量
        self.conv1 = nn.Conv1d(1, width // 2, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm1d(width // 2)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv1d(width // 2, width // 2, kernel_size=3, stride=1, padding=1, bias=False)
